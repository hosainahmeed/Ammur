...
const ProfilePage: React.FC = () => {
  ...
  const InfoCard: React.FC<InfoCardProps> = ({ icon, title, value }) => (
    <div className="flex items-start gap-3 sm:gap-4 bg-white p-3 sm:p-4 rounded-lg shadow-sm">
      <div className="mt-0.5 sm:mt-1 text-blue-600">
        {React.cloneElement(icon as React.ReactElement,)}
      </div>
      <div className="overflow-hidden">
        <h3 className="text-xs sm:text-sm font-medium text-gray-500 truncate">{title}</h3>
        <p className="text-base sm:text-lg font-medium text-gray-800 truncate">{value}</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 pt-20 sm:pt-24 pb-12 sm:pb-16">
        ...
        <div className='flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 mb-8 sm:mb-12'>
          ...
          <div className="md:flex-1 w-full text-center sm:text-left">
            <div className="mb-3 sm:mb-0">
              ...
              <p className="text-base sm:text-lg text-blue-600">{userData.profession}</p>
            </div>
            <Link href={'/profile-setting/edit-profile'}>
              <button className="mt-3 sm:mt-2 cursor-pointer flex w-full sm:w-fit items-center justify-center sm:justify-start gap-2 bg-blue-600 hover:bg-blue-700 text-white py-1.5 sm:py-2 px-3 sm:px-4 rounded-lg transition-colors text-sm sm:text-base">
                <Edit size={16} />
                Edit Profile
              </button>
            </Link>
        ...
        {/* Subscription banner */}
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-3 sm:p-4 rounded-lg mb-6 sm:mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
          <div className="flex items-center gap-2 sm:gap-3">
            <Crown size={20} className="flex-shrink-0" />
            <div>
              <p className="text-xs sm:text-sm font-medium">Active Subscription</p>
              <p className="text-base sm:text-lg font-semibold">{userData.subscription}</p>
            </div>
          </div>
          <Link href={'/pricing'}>
            <button className="bg-white cursor-pointer text-blue-800 py-1 px-2 sm:px-3 rounded text-xs sm:text-sm font-medium hover:bg-blue-50 transition-colors self-end sm:self-auto">
              Manage
            </button>
          </Link>
        </div>
